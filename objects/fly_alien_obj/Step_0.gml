...
if(image_xscale <= MAX_SIZE && curr_state == alien_state_fly.walking){
	image_xscale += 0.028 * alien_speed;
	image_yscale += 0.028 * alien_speed;
}
//have aliens move towards the line.
if(y > -10 && curr_state == alien_state_fly.walking){
	y -= 1 * alien_speed;	
}

// Aliens attacking
if (y <= -10 && curr_state != alien_state_fly.hit && curr_state != alien_state_fly.dying)
{
	curr_state = alien_state_fly.attack;
...
if (keyboard_check_pressed(vk_space) and global.pistolammo >= 1 and place_meeting(x, y, reticle_obj) && curr_state != alien_state_fly.dying)
{
	curr_state = alien_state_fly.hit;
	alien_health -= 5;
	image_index = hit_start
}

if (alien_health <= 0 && curr_state != alien_state_fly.dying)
{
	curr_state = alien_state_fly.dying;
}

// Animation loop
if (curr_state == alien_state_fly.walking)
{
...
else if (curr_state == alien_state_fly.attack)
{
...
else if (curr_state == alien_state_fly.hit)
{
...
else if (curr_state == alien_state_fly.dying)
{
